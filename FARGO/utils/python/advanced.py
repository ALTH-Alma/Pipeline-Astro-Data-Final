...
def vector_field(vx,vy,scale=5,nsn = 3, **karg):
    nsx = nsy = nsn
    ...
    ax = plt.gca()
    ax.quiver(X,Y,U,V,scale=scale,pivot='middle', **karg)
...
def get_random_streams(vx, vy, xmin=None, xmax=None, ymin=None, ymax=None, maxlen=4*np.pi,n=30, nmax=100000):
    if xmin == None:
        xmin = vx.xmin
    if ymin == None:
        ymin = vx.ymin
    ...
    counter = 0
    for x,y in zip(X,Y):
        stream = get_stream(vx, vy, x, y,maxlength=maxlen ,nmax=nmax, bidirectional=True)
...
def plot_random_streams(streams, cartesian=False, **kargs):
    ...
    for stream in streams:
        for sub_stream in stream:
            if cartesian:
                ax.plot(sub_stream[1]*np.cos(sub_stream[0]),sub_stream[1]*np.sin(sub_stream[0]),**kargs)
            else:
