...
if (*SPACING=='N'){
  ...
  for (i = begin_i; i<=NTOT; i++) { // MAIN LOOP
    ...
    // Normalize to [0, 1] and scale to [0.5, 2.0]
    double rand_double = (double)rand_int / RAND_MAX; // Normalize to [0, 1]
    double random_value = 0.5 + rand_double * (2.0 - 0.5); // Scale to [0.5, 2.0]
    DT = random_value*dt0;
